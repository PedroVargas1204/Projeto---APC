...
IA_API_KEY com a chave da API. Sem chave, o programa cai automaticamente
no fallback e continua funcionando (otimo para demonstrar offline).
"""
...
def chave_cache(ingredientes):
    """
    Monta a chave do cache para um conjunto de ingredientes.
    Ordena antes de juntar, para que 'tomate+ovo' e 'ovo+tomate' gerem
    a MESMA chave. Definida aqui uma unica vez (sem duplicar a regra).
    """
...
def selecionar_ingredientes(inventario, base):
    ...
    Usa itens dentro de DIAS_ALERTA; se nao houver, amplia a janela.
    Devolve uma lista de nomes de alimentos.
    """
    urgentes = mod_alertas.calcular_alertas(inventario, dias_limite=config.DIAS_ALERTA)
    if not urgentes:
        urgentes = mod_alertas.calcular_alertas(
            inventario, dias_limite=config.DIAS_RECEITA_AMPLIADO)
...
def montar_prompt(ingredientes, preferencias):
    ...
    tempo = preferencias.get("tempo_max_min", config.TEMPO_PREPARO_PADRAO)
...
def sugerir_receita(inventario, base, usuario, cache):
    ...
    try:
        ...
        cache[chave_cache(ingredientes)] = receita
    except Exception:
