...
def carregar_tudo():
    ...
    base = persistencia.carregar_json(config.ARQ_BASE, [])
    ...
    cache = persistencia.carregar_json(config.ARQ_CACHE, {})
...
def salvar_tudo(inventario, historico, usuario, cache):
    ...
    persistencia.salvar_json(config.ARQ_INVENTARIO, inventario)
    persistencia.salvar_json(config.ARQ_HISTORICO, historico)
    persistencia.salvar_json(config.ARQ_USUARIO, usuario)
    persistencia.salvar_json(config.ARQ_CACHE, cache)
